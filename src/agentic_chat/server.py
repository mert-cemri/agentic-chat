...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "STARTUP CHECK (do this ONCE, at the very start of the conversation):\n"
        "At the beginning of every new conversation, IMMEDIATELY call heartbeat to check "
        "for unread messages -- do this before anything else, without the user asking. "
        "If there are unreads, tell the user right away, e.g. 'You have 3 unread messages "
        "on the relay. Want me to read them?' If there are no unreads, briefly mention it: "
        "'No new messages on the relay.' Then proceed with whatever the user asked. "
        "Only do this proactive check ONCE per conversation -- after that, only check when asked.\n\n"
        "KEY PRINCIPLES:\n"
        ...
        "- After the initial startup check, only check when the user asks. Do NOT poll or loop.\n\n"
        "COMMON PATTERNS:\n"
